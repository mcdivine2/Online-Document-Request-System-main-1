	...
	class class_model{
	    ...
	    public function fetchAll_documentrequest(){ 
		  ...
		  public function fetchAll_releasing(){ 
            $sql = "SELECT * FROM  tbl_documentrequest WHERE library_status = 'Releasing' ";
				$stmt = $this->conn->prepare($sql); 
				$stmt->execute();
				$result = $stmt->get_result();
		        $data = array();
		  ...
		  public function fetchAll_released(){ 
            $sql = "SELECT * FROM  tbl_documentrequest WHERE library_status = 'Released' ";
				$stmt = $this->conn->prepare($sql); 
				$stmt->execute();
				$result = $stmt->get_result();
		        $data = array();
		  ...
		  public function fetchAll_pendingpayment(){ 
            $sql = "SELECT * FROM  tbl_documentrequest WHERE library_status = 'Waiting for Payment'";
				$stmt = $this->conn->prepare($sql); 
				$stmt->execute();
				$result = $stmt->get_result();
		        $data = array();
		...
		public function edit_request($control_no, $student_id, $document_name, $no_ofcopies, $date_request, $date_releasing, $library_status, $request_id){
			$sql = "UPDATE `tbl_documentrequest` SET  `control_no` = ?, `student_id` = ?, `document_name` = ?, `no_ofcopies` = ?, `date_request` = ?, `date_releasing` = ?, `library_status` = ?  WHERE request_id = ?";
			 $stmt = $this->conn->prepare($sql);
			$stmt->bind_param("sssssssi", $control_no, $student_id, $document_name, $no_ofcopies, $date_request, $date_releasing, $library_status, $request_id);
			if($stmt->execute()){
				$stmt->close();
				$this->conn->close();
				return true;
