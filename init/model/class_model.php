...
class class_model
{
	...
	public function add_payment($trace_no, $ref_no, $control_no, $document_name, $date_ofpayment, $total_amount, $payment_proof, $student_id)
	{
		$stmt = $this->conn->prepare("INSERT INTO `tbl_payment` (`trace_no`, `ref_no`, `control_no`, `document_name`, `date_ofpayment`, `total_amount`, `proof_ofpayment`, `student_id`, `status`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, 'Paid')") or die($this->conn->error);
	...
	public function prepare($sql)
	{
		try {
			$stmt = $this->conn->prepare($sql);
			if ($stmt === false) {
				throw new Exception("Error preparing statement: " . htmlspecialchars($this->conn->error));
			}
			return $stmt;
		} catch (Exception $e) {
			// Handle errors
	...
	public function close()
	{
		if ($this->conn) {
			$this->conn->close();
			$this->conn = null;  // Clear the connection
		}
	}
	public function executeStatement($stmt, $params = [])
	{
		try {
			...
			return $stmt;  // Return the statement for further use if needed
		} catch (Exception $e) {
			echo '<div class="alert alert-danger">' . $e->getMessage() . '</div>';
			return false;
		}
